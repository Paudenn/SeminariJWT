...
class AuthRoutes{
    ...
    public async register(req: Request, res: Response){
        const {name,birthDate, email, password,booksPublished, username, roles} = req.body;

        //encrypt password
        const salt = await bcrypt.genSalt(10);
        const hashed = await bcrypt.hash(password, salt);
        const newUser = new Writer({name,birthDate, email, password,booksPublished: hashed, username, roles});
    ...
    public async login(req: Request, res: Response) {
        const userFound = await Writer.findOne({email: req.body.email}).populate('roles');
